...
def p_create_matrix(p):
    """expression : ZEROES '(' expression ')'
                  | EYE '(' expression ')'
                  | ONES '(' expression ')' """
    p[0] = ast.MatrixCreation(p[1], p[3], p.lineno(1))
...
def p_assign_expression(p):
    ...
    if len(p) == 4:
        p[0] = ast.AssignExpr(p[1], p[2], p[3], p.lineno(1))
    elif len(p) == 7:
        array_el = ast.ArrayElement(p[1], p[3], p.lineno(1))
        p[0] = ast.AssignExpr(array_el, p[5], p[6], p.lineno(1))
    elif len(p) == 9:
        array_el = ast.Array2DElement(p[1], p[3], p[5], p.lineno(1))
        p[0] = ast.AssignExpr(array_el, p[7], p[8], p.lineno(1))
...
def p_if_statement(p):
    ...
    p[0] = ast.IfStmt(p[3], p[5], p.lineno(1))


def p_if_else_statement(p):
    """statement : IF '(' expression ')' statement ELSE statement"""
    p[0] = ast.IfStmt(p[3], p[5], p.lineno(1), p[7] )
